...
void Adafruit_RGBLCDShield::begin(uint8_t cols, uint8_t lines, uint8_t dotsize) {
	...
	if(_i2cAddr != 255) {
		...
		pinMode(PIN_BASE + 8, OUTPUT);
		pinMode(PIN_BASE + 6, OUTPUT);
		pinMode(PIN_BASE + 7, OUTPUT);
		setBacklight(0x7);

		if (_rw_pin)
			pinMode(PIN_BASE + _rw_pin, OUTPUT);

		pinMode(PIN_BASE + _rs_pin, OUTPUT);
		pinMode(PIN_BASE + _enable_pin, OUTPUT);
		for (uint8_t i = 0; i < 4; i++) 
			pinMode(PIN_BASE + _data_pins[i], OUTPUT);

		for (uint8_t i = 0; i < 5; i++) {
			pinMode(PIN_BASE + _button_pins[i], INPUT);
			pullUpDnControl(PIN_BASE + _button_pins[i], PUD_UP);
		}
	}
...
// Aus der Arduino Library print.cpp (angepasst)
size_t Adafruit_RGBLCDShield::print(const string &s)
{
	return write(s.c_str(), s.length());
}

size_t Adafruit_RGBLCDShield::print(const char str[])
{
	return write(str);
}

size_t Adafruit_RGBLCDShield::print(char c)
{
	return write(c);
}

size_t Adafruit_RGBLCDShield::print(int n, int base)
{
	return print((long) n, base);
}

size_t Adafruit_RGBLCDShield::print(unsigned int n, int base)
{
	return print((unsigned long) n, base);
}

size_t Adafruit_RGBLCDShield::print(long n, int base)
{
	if (base == 0) {
		return write(n);
	}
...
size_t Adafruit_RGBLCDShield::print(unsigned long n, int base)
{
	if (base == 0) return write(n);
	else return printNumber(n, base);
}

size_t Adafruit_RGBLCDShield::print(double n, int digits)
{
	return printFloat(n, digits);
}

/***** Private Funktionen aus print.cpp *****/

size_t Adafruit_RGBLCDShield::printNumber(unsigned long n, uint8_t base)
{
...
size_t Adafruit_RGBLCDShield::printFloat(double number, uint8_t digits) 
{ 
	size_t n = 0;
...
void  Adafruit_RGBLCDShield::_digitalWrite(uint8_t p, uint8_t d) {
	digitalWrite(PIN_BASE + p, d);
}

// Allows to set the backlight, if the LCD backpack is used
void Adafruit_RGBLCDShield::setBacklight(uint8_t status) {
	// check if i2c or SPI
	digitalWrite(PIN_BASE + 8, ~(status >> 2) & 0x1);
	digitalWrite(PIN_BASE + 7, ~(status >> 1) & 0x1);
	digitalWrite(PIN_BASE + 6, ~status & 0x1);
}

// little wrapper for i/o directions
void  Adafruit_RGBLCDShield::_pinMode(uint8_t p, uint8_t d) {
	pinMode(PIN_BASE + p, d);
}
...
uint8_t Adafruit_RGBLCDShield::readButtons(void) {
	...
	for (uint8_t i = 0; i < 5; i++) {
		reply &= ~((digitalRead(PIN_BASE + _button_pins[i])) << i);
	}
	return reply;
}
